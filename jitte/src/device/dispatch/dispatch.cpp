...
void Dispatch::process_cmd() {
    ...
    case CQ_DISPATCH_CMD_REMOTE_WRITE:
        if (DIAG) {
            printf("cmd_remote_write\n");
        }
        process_remote_write();
        break;

    case CQ_DISPATCH_CMD_SET_WRITE_OFFSET:
        if (DIAG) {
            printf("cmd_set_write_offset\n");
        }
        process_set_write_offset();
        break;
...
void Dispatch::process_write_linear(bool multicast, uint32_t num_mcast_dests) {
    ...
    assert(write_offset_index < 3);
    uint32_t dst_addr = cmd->write_linear.addr + m_write_offset[write_offset_index];
...
void Dispatch::process_write_packed(bool mcast, uint32_t flags) {
    ...
    assert(write_offset_index < 3);
    uint32_t dst_addr = cmd->write_packed.addr + m_write_offset[write_offset_index];

    uint32_t data_start = sizeof(CQDispatchCmd) + count * sub_cmd_size;
    data_start = round_up_pow2(data_start, L1_ALIGNMENT);
    const uint8_t *data_ptr = m_cmd_ptr + data_start;

    uint32_t padded_xfer_size = round_up_pow2(xfer_size, L1_ALIGNMENT);
    uint32_t stride = 
...
void Dispatch::process_write_packed_large() {
    const CQDispatchCmd *cmd = reinterpret_cast<const CQDispatchCmd *>(m_cmd_ptr);
    ...
    assert(write_offset_index < 3);
    uint32_t local_write_offset = m_write_offset[write_offset_index];
    uint32_t data_start = 
        sizeof(CQDispatchCmd) + count * sizeof(CQDispatchWritePackedLargeSubCmd);
    data_start = round_up_pow2(data_start, L1_ALIGNMENT);
    const uint8_t *data_ptr = m_cmd_ptr + data_start;

    // no need for L1 cache in emulator context
    const CQDispatchWritePackedLargeSubCmd *sub_cmd_ptr = 
        reinterpret_cast<const CQDispatchWritePackedLargeSubCmd *>(
            m_cmd_ptr + sizeof(CQDispatchCmd));

    uint32_t dst_noc = 0; // will be set later
    ...
    while (count != 0) {
        ...
        if (init_state) {
            dst_noc = sub_cmd_ptr->noc_xy_addr;
        }

        sub_cmd_ptr++;

        uint64_t dst = m_noc.get_noc_addr_helper(dst_noc, dst_addr);
        m_noc.write_multicast(data_ptr, dst, length, num_dests);
...
void Dispatch::process_remote_write() {
    // nothing to do so far
    m_cmd_ptr += sizeof(CQDispatchCmd);
}

void Dispatch::process_set_write_offset() {
    const CQDispatchCmd *cmd = reinterpret_cast<const CQDispatchCmd *>(m_cmd_ptr);
    m_write_offset[0] = cmd->set_write_offset.offset0;
    m_write_offset[1] = cmd->set_write_offset.offset1;
    m_write_offset[2] = cmd->set_write_offset.offset2;
    m_cmd_ptr += sizeof(CQDispatchCmd);
}
