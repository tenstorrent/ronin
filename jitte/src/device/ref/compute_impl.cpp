...
uint32_t ComputeImpl::get_arg_uint32(int arg_idx) {
    uint32_t *arg_base = BaseAddr::get_trisc_arg_base(m_l1);
...
//
//    ACHTUNG: Following four methods are experimental and have no direct equivalent in TT-Metal.
//        In 0.48 this functionality is used in only one kernel and expressed using ecxplicit
//        UNPACK() / PACK() macros thus fully breaking fundamental concepts of Compute API
//        that assume hiding all features specific to any of TRISC cores.
//

uint32_t ComputeImpl::get_read_ptr(uint32_t cbid) {
    // TT-Metal mapping: performed by UNPACK only (may return 0 on other TRISC cores)
    return m_cb->get_read_ptr(cbid);
}

uint32_t ComputeImpl::get_write_ptr(uint32_t cbid) {
    // TT-Metal mapping: performed by PACK only (may return 0 on other TRISC cores)
    return m_cb->get_write_ptr(cbid);
}

void ComputeImpl::set_read_ptr(uint32_t cbid, uint32_t ptr) {
    // TT-Metal mapping: performed by UNPACK only
    m_cb->set_read_ptr(cbid, ptr);
}

void ComputeImpl::set_write_ptr(uint32_t cbid, uint32_t ptr) {
    // TT-Metal mapping: performed by PACK only
    m_cb->set_write_ptr(cbid, ptr);
}
...
void ComputeImpl::pack_untilize_init(uint32_t icb, uint32_t ocb, uint32_t block_ct_dim) {
    // nothing to do
}

void ComputeImpl::pack_untilize_block(
        uint32_t icb, 
        uint32_t block_rt_dim, 
        uint32_t ocb,
        uint32_t block_ct_dim) {
    // ACHTUNG: Not clear how original code is supposed to work if block_rt_dim > 1
    //     (as unpack_A unpacks same slice for each row)
    assert(block_rt_dim == 1);
    for (uint32_t r = 0; r < block_rt_dim; r++) {
        for (uint32_t c = 0; c < block_ct_dim; c++) {
            m_llk.unpack_A(icb, c, false);
            m_llk.math_eltwise_unary_datacopy(c);
        }
        m_llk.pack_untilize(1, ocb, 0, block_ct_dim, block_ct_dim);
    }
}

void ComputeImpl::pack_untilize_uninit(uint32_t ocb) {
    // nothing to do
}

void ComputeImpl::pack_untilize_dst_init_short(
        uint32_t ocb, 
        uint32_t face_r_dim, 
        uint32_t num_faces,
        uint32_t block_ct_dim, 
...
void ComputeImpl::pack_untilize_dst(
        uint32_t ocb, 
        uint32_t block_rt_dim, 
...
void ComputeImpl::pack_untilize_init_short(uint32_t icb, uint32_t ocb, uint32_t block_ct_dim) {
    // nothing to do
}
...
void ComputeImpl::max_tile_init() {
    // nothing to do
}

void ComputeImpl::max_tile(uint32_t idst0, uint32_t idst1) {
    m_llk.math_eltwise_unary_sfpu_max(idst0, idst1);
}
...
void ComputeImpl::binop_with_scalar_tile_init() {
    // nothing to do
}

void ComputeImpl::add_unary_tile(uint32_t idst, uint32_t param0) {
    m_llk.math_eltwise_unary_sfpu_add_scalar(idst, param0);
}

void ComputeImpl::sub_unary_tile(uint32_t idst, uint32_t param0) {
    m_llk.math_eltwise_unary_sfpu_sub_scalar(idst, param0);
}

void ComputeImpl::mul_unary_tile(uint32_t idst, uint32_t param0) {
    m_llk.math_eltwise_unary_sfpu_mul_scalar(idst, param0);
}

void ComputeImpl::div_unary_tile(uint32_t idst, uint32_t param0) {
    m_llk.math_eltwise_unary_sfpu_div_scalar(idst, param0);
}

void ComputeImpl::rsub_unary_tile(uint32_t idst, uint32_t param0) {
...
void ComputeImpl::tanto_max_init() {
    // nothing to do
}
...
void ComputeImpl::tanto_unpack_reduce_rows_init(uint32_t icb0, uint32_t icb1) {
    // nothing to do
}

void ComputeImpl::tanto_unpack_reduce_cols_init(uint32_t icb0, uint32_t icb1) {
    // nothing to do
}

void ComputeImpl::tanto_unpack_reduce_scalar_init(uint32_t icb0, uint32_t icb1) {
    // nothing to do
}
...
void ComputeImpl::tanto_pack_row_init(uint32_t ocb) {
    // nothing to do
}

void ComputeImpl::tanto_pack_col_init(uint32_t ocb) {
    // nothing to do
}

void ComputeImpl::tanto_pack_scalar_init(uint32_t ocb) {
    // nothing to do
}
