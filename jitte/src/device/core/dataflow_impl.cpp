...
uint32_t DataflowImpl::get_arg_uint32(int arg_idx) {
    uint32_t *arg_base = 
        (m_noc_index == 0) ?
            BaseAddr::get_brisc_arg_base(m_l1) :
            BaseAddr::get_ncrisc_arg_base(m_l1);
...
uint32_t DataflowImpl::get_semaphore(uint32_t semaphore_id) {
    // TODO: Make L1_ALIGNMENT arch-specific
    uint32_t L1_ALIGNMENT = 16;
    return BaseAddr::get_semaphore_base(m_l1) + semaphore_id * L1_ALIGNMENT;
}
...
void DataflowImpl::noc_async_read_global_dram(
    ...
    uint32_t limit = page_size - src_offset % page_size;
    while (bytes_left != 0) {
        ...
        noc_fast_read_global(dst_addr, noc_addr, xfer_bytes);
        dst_addr += xfer_bytes;
...
void DataflowImpl::noc_async_write_global_dram(
    ...
    uint32_t bytes_left = len_bytes;
    uint32_t limit = page_size - dst_offset % page_size;
    while (bytes_left != 0) {
        uint32_t xfer_bytes = (bytes_left < limit) ? bytes_left : limit;
        uint64_t noc_addr = get_noc_addr_global_dram(dst_addr, dst_log2_page_size, dst_offset);
        noc_fast_write_global(src_addr, noc_addr, xfer_bytes);
        src_addr += xfer_bytes;
