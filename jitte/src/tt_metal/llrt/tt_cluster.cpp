...
void Cluster::detect_arch_and_target() {
    this->target_type_ = TargetDevice::Ronin;

    auto arch_env = getenv("ARCH_NAME");
    TT_FATAL(arch_env, "arch_env needs to be set for emulator (ARCH_NAME=)");
...
void Cluster::generate_cluster_descriptor() {
    TT_FATAL(this->target_type_ == TargetDevice::Ronin, "Unsupported target type");
...
void Cluster::write_core(
    ...
    // ACHTUNG: Iterators must pertain to the same collection instance
    auto target_remote_device_ids = this->get_driver(chip_id).get_target_remote_device_ids();
    if (target_remote_device_ids.find(virtual_core.chip) != target_remote_device_ids.end()) {
...
void Cluster::write_reg(const std::uint32_t *mem_ptr, tt_cxy_pair target, uint64_t addr) const {
    ...
    // ACHTUNG: Iterators must pertain to the same collection instance
    auto target_remote_device_ids = this->get_driver(chip_id).get_target_remote_device_ids();
    if (target_remote_device_ids.find(virtual_target.chip) != target_remote_device_ids.end()) {
        this->get_driver(chip_id).wait_for_non_mmio_flush();
    }
}
