...
#if 0 // TODO: Figure out why this is not compiled on MSVC
    return fmt::format("eth_{}_{}", magic_enum::enum_name(this->config_.noc), this->config_.eth_mode);
#endif
    return fmt::format("eth_{}_{}", magic_enum::enum_name(this->config_.noc), uint8_t(this->config_.eth_mode));
}
...
void Kernel::validate_runtime_args_size(
    ...
    if (total_rt_args > max_rt_args) {
        log_warning(tt::LogMetal, "Too many runtime args, unique: {} common: {} on {}", num_unique_rt_args, num_common_rt_args, uint8_t(this->processor()));
