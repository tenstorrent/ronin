...
void golden_matmul(
        ...
        uint32_t K, 
        uint32_t B) {
    // ACHTUNG: B is ignored (assumed B == 1)
...
#endif
        std::vector<bfloat16> src0_vec = create_random_vector_of_bfloat16_native(dram_buffer_A_size, 1, 123, -0.4f);
        std::vector<bfloat16> src1_vec = create_random_vector_of_bfloat16_native(dram_buffer_B_size, 1, 12522, -0.3f);
