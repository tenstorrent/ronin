...
using ronin::nn::mobilenetv2_050::tanto::MobileNetV2_050_GlobalDsc;
...
class MobileNetV2_050_GlobalDscRunner: public util::NetRunner {
public:
    MobileNetV2_050_GlobalDscRunner(const core::Device &device, int algo_batch_size);
    ~MobileNetV2_050_GlobalDscRunner();
protected:
...
private:
    ...
    std::unique_ptr<MobileNetV2_050_GlobalDsc> m_net;
    std::unique_ptr<util::NetWrap<MobileNetV2_050_GlobalDsc>> m_iface;
};

MobileNetV2_050_GlobalDscRunner::MobileNetV2_050_GlobalDscRunner(
...
void MobileNetV2_050_GlobalDscRunner::reorder_inputs() {
    m_inputs[0] = util::reorder_nchw_to_nhwc(m_inputs[0], m_batch_size, 224, 224, 3);
}

void MobileNetV2_050_GlobalDscRunner::infer_batch_size() {
    if (m_inputs.size() == 0) {
...
void MobileNetV2_050_GlobalDscRunner::create_net() {
    m_net = std::make_unique<MobileNetV2_050_GlobalDsc>(m_device, m_batch_size, m_algo_batch_size);
    m_iface = std::make_unique<util::NetWrap<MobileNetV2_050_GlobalDsc>>(m_net.get());
    set_iface(m_iface.get());
}

void MobileNetV2_050_GlobalDscRunner::init_net(const std::string &data_dir) {
    m_net->init(data_dir);
}

void MobileNetV2_050_GlobalDscRunner::sync_run() {
    core::Queue queue(m_device, 0);
    queue.finish();
}

void MobileNetV2_050_GlobalDscRunner::print_outputs() {
    std::vector<float> output;
    m_net->get_output(0, output);
    util::print_imagenet_classes(output, m_input_batch_size, 5);
}

void MobileNetV2_050_GlobalDscRunner::pad_inputs_to_batch_size() {
...
void run_global_dsc(const util::NetCmdArgs &args) {
    ...
    MobileNetV2_050_GlobalDscRunner runner(device, algo_batch_size);
    runner.run(args);
