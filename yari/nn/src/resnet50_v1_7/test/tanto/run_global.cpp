...
using ronin::nn::resnet50_v1_7::tanto::ResNet50V17Global;
...
class ResNet50V17GlobalRunner: public util::NetRunner {
public:
    ResNet50V17GlobalRunner(const core::Device &device, int algo_batch_size);
    ~ResNet50V17GlobalRunner();
protected:
...
private:
    ...
    std::unique_ptr<ResNet50V17Global> m_net;
    std::unique_ptr<util::NetWrap<ResNet50V17Global>> m_iface;
};

ResNet50V17GlobalRunner::ResNet50V17GlobalRunner(
...
void ResNet50V17GlobalRunner::reorder_inputs() {
    m_inputs[0] = util::reorder_nchw_to_nhwc(m_inputs[0], m_batch_size, 224, 224, 3);
}

void ResNet50V17GlobalRunner::infer_batch_size() {
    if (m_inputs.size() == 0) {
...
void ResNet50V17GlobalRunner::create_net() {
    m_net = std::make_unique<ResNet50V17Global>(m_device, m_batch_size, m_algo_batch_size);
    m_iface = std::make_unique<util::NetWrap<ResNet50V17Global>>(m_net.get());
    set_iface(m_iface.get());
}

void ResNet50V17GlobalRunner::init_net(const std::string &data_dir) {
    m_net->init(data_dir);
}

void ResNet50V17GlobalRunner::sync_run() {
    core::Queue queue(m_device, 0);
    queue.finish();
}

void ResNet50V17GlobalRunner::print_outputs() {
    std::vector<float> output;
    m_net->get_output(0, output);
    util::print_imagenet_classes(output, m_input_batch_size, 5);
}

void ResNet50V17GlobalRunner::pad_inputs_to_batch_size() {
...
void run_global(const util::NetCmdArgs &args) {
    ...
    ResNet50V17GlobalRunner runner(device, algo_batch_size);
    runner.run(args);
