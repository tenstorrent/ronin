...
ResNet18Ref::ResNet18Ref(int N):
        NetRef(N) { }

ResNet18Ref::~ResNet18Ref() { }

void ResNet18Ref::init(const std::string &data_dir) {
    NetRef::set_data_dir(data_dir);
    init_layers();
    load_buffers();
}

int ResNet18Ref::input_count() {
    return 1;
}

void ResNet18Ref::set_input(int index, const std::vector<float> &data) {
    assert(index == 0);
    int buffer_index = 6;
...
int ResNet18Ref::output_count() {
    return 1;
}

void ResNet18Ref::get_output(int index, std::vector<float> &data) {
    assert(index == 0);
    int buffer_index = 91;
    int volume = m_N * 1000;
    data.resize(volume);
...
void ResNet18Ref::run() {
    NetRef::run();
}

void ResNet18Ref::init_layers() {
...
void ResNet18Ref::init_max_pool2d(
        int ix,
        int iy,
...
void ResNet18Ref::init_reduce_mean(
        int ix,
        int iy,
