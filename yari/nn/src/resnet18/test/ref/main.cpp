...
private:
    std::unique_ptr<ResNet18Ref> m_net;
    std::unique_ptr<util::NetWrap<ResNet18Ref>> m_iface;
};

ResNet18RefRunner::ResNet18RefRunner() { }

ResNet18RefRunner::~ResNet18RefRunner() { }

void ResNet18RefRunner::reorder_inputs() {
    m_inputs[0] = util::reorder_nchw_to_nhwc(m_inputs[0], m_batch_size, 224, 224, 3);
}

void ResNet18RefRunner::infer_batch_size() {
...
void ResNet18RefRunner::create_net() {
    m_net = std::make_unique<ResNet18Ref>(m_batch_size);
    m_iface = std::make_unique<util::NetWrap<ResNet18Ref>>(m_net.get());
    set_iface(m_iface.get());
}

void ResNet18RefRunner::init_net(const std::string &data_dir) {
    m_net->init(data_dir);
}

void ResNet18RefRunner::print_outputs() {
