...
using ronin::nn::resnet18::tanto::ResNet18Mixed;
...
class ResNet18MixedRunner: public util::NetRunner {
public:
    ResNet18MixedRunner(const core::Device &device, int algo_batch_size);
    ~ResNet18MixedRunner();
public:
...
private:
    ...
    std::unique_ptr<ResNet18Mixed> m_net;
    std::unique_ptr<util::NetWrap<ResNet18Mixed>> m_iface;
};

ResNet18MixedRunner::ResNet18MixedRunner(
...
void ResNet18MixedRunner::run(const util::NetCmdArgs &args) {
    // special version that benchmarks only part running on device
    reset(args);
    read_inputs();
    infer_batch_size();
    reorder_inputs();
...
void ResNet18MixedRunner::reorder_inputs() {
    m_inputs[0] = util::reorder_nchw_to_nhwc(m_inputs[0], m_batch_size, 224, 224, 3);
}

void ResNet18MixedRunner::infer_batch_size() {
    if (m_inputs.size() == 0) {
...
void ResNet18MixedRunner::create_net() {
    m_net = std::make_unique<ResNet18Mixed>(m_device, m_batch_size, m_algo_batch_size);
    m_iface = std::make_unique<util::NetWrap<ResNet18Mixed>>(m_net.get());
    set_iface(m_iface.get());
}

void ResNet18MixedRunner::init_net(const std::string &data_dir) {
    m_net->init(data_dir);
}

void ResNet18MixedRunner::print_outputs() {
    std::vector<float> output;
    m_net->get_output(0, output);
    util::print_imagenet_classes(output, m_input_batch_size, 5);
}

void ResNet18MixedRunner::pad_inputs_to_batch_size() {
...
void run_mixed(const util::NetCmdArgs &args) {
    ...
    ResNet18MixedRunner runner(device, algo_batch_size);
    runner.run(args);
