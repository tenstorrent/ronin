...
void kernel_main() {
  ...
  uint32 PQ_full = get_arg_val<uint32>(10);
  uint32 PQ_tail = get_arg_val<uint32>(11);
  uint32 RSK = get_arg_val<uint32>(12);
  uint32 y_pos = get_arg_val<uint32>(13);
  uint32 y_stride = get_arg_val<uint32>(14);
  kernel(gw, gy, pw, py, N, K, PQ_full, PQ_tail, RSK, y_pos, y_stride);
}
