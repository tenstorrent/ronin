...
void kernel(Global gw, Global gw2, Global gz, Global gy, Pipe pw, Pipe pw2,
            Pipe pz, Pipe py, uint32 N, uint32 C, uint32 K, uint32 KC,
            uint32 PQ, uint32 RSC, uint32 y_pos, uint32 y_stride) {
...
void kernel_main() {
  ...
  Global gz;
  gz.addr = get_arg_val<uint32>(4);
  gz.log2_page_size = get_arg_val<uint32>(5);
  Global gy;
  gy.addr = get_arg_val<uint32>(6);
  gy.log2_page_size = get_arg_val<uint32>(7);
  Pipe pw;
  pw.cb_id = get_arg_val<uint32>(8);
  pw.frame_size = get_arg_val<uint32>(9);
  Pipe pw2;
  pw2.cb_id = get_arg_val<uint32>(10);
  pw2.frame_size = get_arg_val<uint32>(11);
  Pipe pz;
  pz.cb_id = get_arg_val<uint32>(12);
  pz.frame_size = get_arg_val<uint32>(13);
  Pipe py;
  py.cb_id = get_arg_val<uint32>(14);
  py.frame_size = get_arg_val<uint32>(15);
  uint32 N = get_arg_val<uint32>(16);
  uint32 C = get_arg_val<uint32>(17);
  uint32 K = get_arg_val<uint32>(18);
  uint32 KC = get_arg_val<uint32>(19);
  uint32 PQ = get_arg_val<uint32>(20);
  uint32 RSC = get_arg_val<uint32>(21);
  uint32 y_pos = get_arg_val<uint32>(22);
  uint32 y_stride = get_arg_val<uint32>(23);
  kernel(gw, gw2, gz, gy, pw, pw2, pz, py, N, C, K, KC, PQ, RSC, y_pos,
         y_stride);
}
