...
void kernel(Pipe px, Pipe pw, Pipe pb, Pipe pw2, Pipe pb2, Pipe py, Pipe pu_im,
            Pipe pt_im, Pipe py_im, uint32 N, uint32 C, uint32 K, uint32 Co,
            uint32 Ci, uint32 Ko, uint32 Ki, uint32 KC, uint32 PQ, uint32 RS,
            uint32 RSC, uint32 unary_param0) {
  tanto_relu_max_init();
  uint32 Ct = C / 32;
  uint32 Kt = K / 32;
  px.frame_size = Ct;
...
void MAIN {
  ...
  Pipe pu_im;
  pu_im.cb_id = get_arg_val<uint32>(12);
  pu_im.frame_size = get_arg_val<uint32>(13);
  Pipe pt_im;
  pt_im.cb_id = get_arg_val<uint32>(14);
  pt_im.frame_size = get_arg_val<uint32>(15);
  Pipe py_im;
  py_im.cb_id = get_arg_val<uint32>(16);
  py_im.frame_size = get_arg_val<uint32>(17);
  uint32 N = get_arg_val<uint32>(18);
  uint32 C = get_arg_val<uint32>(19);
  uint32 K = get_arg_val<uint32>(20);
  uint32 Co = get_arg_val<uint32>(21);
  uint32 Ci = get_arg_val<uint32>(22);
  uint32 Ko = get_arg_val<uint32>(23);
  uint32 Ki = get_arg_val<uint32>(24);
  uint32 KC = get_arg_val<uint32>(25);
  uint32 PQ = get_arg_val<uint32>(26);
  uint32 RS = get_arg_val<uint32>(27);
  uint32 RSC = get_arg_val<uint32>(28);
  uint32 unary_param0 = get_arg_val<uint32>(29);
  tanto_compute_init();
  kernel(px, pw, pb, pw2, pb2, py, pu_im, pt_im, py_im, N, C, K, Co, Ci, Ko, Ki,
         KC, PQ, RS, RSC, unary_param0);
}
} // namespace NAMESPACE
