...
void read_px(Local lx, Local lzero, Pipe px, uint32 C, uint32 start_q,
             uint32 delta_p, uint32 delta_q, uint32 end_q, uint32 x_base,
  ...
  for (uint32 i = 0; i < 32; i++) {
    if ((mask & 1) == 0) {
      ...
      uint32 src_pos = p_term + q_term + r_term + s_term - x_base;
      noc_async_read_one_packet_with_state(
...
void kernel(Global gx, Global gb, Global gzero, Global gmask, Local lx,
            Local lzero, Local lmask, Pipe px, Pipe pb, uint32 N, uint32 C,
            uint32 K, uint32 R, uint32 S, uint32 PQ, uint32 start_p,
            uint32 start_q, uint32 delta_p, uint32 delta_q, uint32 delta_r,
            uint32 delta_s, uint32 end_q, uint32 zero_size, uint32 mask_size,
            uint32 x_pos, uint32 x_stride, uint32 x_base, uint32 x_size,
            uint32 mask_pos) {
    ...
    for (uint32 pq_start = 0; pq_start < PQ; pq_start += 32) {
      ...
      for (uint32 r = 0; r < R; r++) {
        ...
        for (uint32 s = 0; s < S; s++) {
          ...
          read_px(lx, lzero, px, C, start_q, delta_p, delta_q, end_q, x_base,
...
void kernel_main() {
  ...
  Local lzero;
  lzero.addr = get_arg_val<uint32>(9);
  Local lmask;
  lmask.addr = get_arg_val<uint32>(10);
  Pipe px;
  ...
  uint32 R = get_arg_val<uint32>(18);
  uint32 S = get_arg_val<uint32>(19);
  uint32 PQ = get_arg_val<uint32>(20);
  uint32 start_p = get_arg_val<uint32>(21);
  uint32 start_q = get_arg_val<uint32>(22);
  uint32 delta_p = get_arg_val<uint32>(23);
  uint32 delta_q = get_arg_val<uint32>(24);
  uint32 delta_r = get_arg_val<uint32>(25);
  uint32 delta_s = get_arg_val<uint32>(26);
  uint32 end_q = get_arg_val<uint32>(27);
  uint32 zero_size = get_arg_val<uint32>(28);
  uint32 mask_size = get_arg_val<uint32>(29);
  uint32 x_pos = get_arg_val<uint32>(30);
  uint32 x_stride = get_arg_val<uint32>(31);
  uint32 x_base = get_arg_val<uint32>(32);
  uint32 x_size = get_arg_val<uint32>(33);
  uint32 mask_pos = get_arg_val<uint32>(34);
  kernel(gx, gb, gzero, gmask, lx, lzero, lmask, px, pb, N, C, K, R, S, PQ,
         start_p, start_q, delta_p, delta_q, delta_r, delta_s, end_q, zero_size,
         mask_size, x_pos, x_stride, x_base, x_size, mask_pos);
}
