...
void kernel(Global gx, Global gb, Global gb2, Global gzero, Global gmask,
            Local lzero, Local lmask, Pipe px, Pipe pb, Pipe pb2, uint32 N,
            uint32 C, uint32 K, uint32 R, uint32 S, uint32 PQ, uint32 start_p,
            uint32 start_q, uint32 delta_p, uint32 delta_q, uint32 delta_r,
            uint32 delta_s, uint32 end_q, uint32 zero_size, uint32 mask_size,
            uint32 x_pos, uint32 x_stride) {
...
void kernel_main() {
  ...
  Local lmask;
  lmask.addr = get_arg_val<uint32>(11);
  Pipe px;
  px.cb_id = get_arg_val<uint32>(12);
  px.frame_size = get_arg_val<uint32>(13);
  Pipe pb;
  pb.cb_id = get_arg_val<uint32>(14);
  pb.frame_size = get_arg_val<uint32>(15);
  Pipe pb2;
  pb2.cb_id = get_arg_val<uint32>(16);
  pb2.frame_size = get_arg_val<uint32>(17);
  uint32 N = get_arg_val<uint32>(18);
  uint32 C = get_arg_val<uint32>(19);
  uint32 K = get_arg_val<uint32>(20);
  uint32 R = get_arg_val<uint32>(21);
  uint32 S = get_arg_val<uint32>(22);
  uint32 PQ = get_arg_val<uint32>(23);
  uint32 start_p = get_arg_val<uint32>(24);
  uint32 start_q = get_arg_val<uint32>(25);
  uint32 delta_p = get_arg_val<uint32>(26);
  uint32 delta_q = get_arg_val<uint32>(27);
  uint32 delta_r = get_arg_val<uint32>(28);
  uint32 delta_s = get_arg_val<uint32>(29);
  uint32 end_q = get_arg_val<uint32>(30);
  uint32 zero_size = get_arg_val<uint32>(31);
  uint32 mask_size = get_arg_val<uint32>(32);
  uint32 x_pos = get_arg_val<uint32>(33);
  uint32 x_stride = get_arg_val<uint32>(34);
  kernel(gx, gb, gb2, gzero, gmask, lzero, lmask, px, pb, pb2, N, C, K, R, S,
         PQ, start_p, start_q, delta_p, delta_q, delta_r, delta_s, end_q,
         zero_size, mask_size, x_pos, x_stride);
}
