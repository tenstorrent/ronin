...
//
//    GroupConv2dDwSpatial
//

GroupConv2dDwSpatial::GroupConv2dDwSpatial(
        int N,
        int H,
        int W,
        int C,
...
void GroupConv2dDwSpatial::init(
...
void GroupConv2dDwSpatial::run() {
    core::Queue queue(m_device, 0);
    queue.enqueue_program(m_program, false);
}

int GroupConv2dDwSpatial::input_volume(int index) {
    switch (index) {
    case 0:
...
int GroupConv2dDwSpatial::output_volume(int index) {
    assert(index == 0);
    return m_N * u32_align(m_P * m_Q, 32) * m_K;
}

std::vector<float> GroupConv2dDwSpatial::transform_input(int index, const std::vector<float> &x) {
    std::vector<float> y;
    switch (index) {
    case 0:
...
std::vector<float> GroupConv2dDwSpatial::transform_output(int index, const std::vector<float> &x) {
    assert(index == 0);
    return util::unpad(x, m_N, u32_align(m_P * m_Q, 32), m_K, m_N, m_P * m_Q, m_K_arg);
}

void GroupConv2dDwSpatial::init_config() {
...
void GroupConv2dDwSpatial::init_options() {
    m_options = 0;
    if (!m_gb.is_null()) {
        m_options |= OPT_BIAS;
    }
...
void GroupConv2dDwSpatial::validate_globals() {
...
void GroupConv2dDwSpatial::create_globals() {
...
void GroupConv2dDwSpatial::create_locals() {
    if (m_cache_lx) {
...
void GroupConv2dDwSpatial::create_pipes() {
    m_px =
        core::Pipe(
            m_program,
            m_grid,
...
void GroupConv2dDwSpatial::create_semaphores() {
    if (m_mcast) {
...
void GroupConv2dDwSpatial::create_kernels() {
...
void GroupConv2dDwSpatial::create_bias_reader() {
    std::string path = m_kernel_base_path + "/dw_spatial_bias_reader.cpp";
    m_reader = 
        core::Kernel(
            m_program, 
            m_grid, 
...
void GroupConv2dDwSpatial::create_lx_bias_reader() {
    std::string path = m_kernel_base_path + "/dw_spatial_lx_bias_reader.cpp";
    m_reader = 
        core::Kernel(
            m_program, 
            m_grid, 
...
void GroupConv2dDwSpatial::create_writer() {
    std::string path = m_kernel_base_path + "/dw_spatial_writer.cpp";
    m_writer = 
        core::Kernel(
            m_program, 
            m_grid, 
...
*/
    ...
    uint32_t PQ_full = split_PQ / 32;
    uint32_t PQ_tail = (split_PQ % 32) * m_K;
    uint32_t y_stride = (m_batch_size * PQ_rnd - PQ_full * 32) * m_K;
    std::vector<core::KernelArg> args{
        m_gw,
        m_gy,
        m_pw,
...
void GroupConv2dDwSpatial::create_mcast_writer() {
    std::string path = m_kernel_base_path + "/dw_spatial_mcast_writer.cpp";
    m_writer = 
        core::Kernel(
            m_program, 
            m_grid, 
...
void GroupConv2dDwSpatial::create_bias_math() {
...
void GroupConv2dDwSpatial::create_rm_bias_math() {
...
void GroupConv2dDwSpatial::create_bias_unary_math() {
...
void GroupConv2dDwSpatial::create_rm_bias_unary_math() {
    std::string suffix = get_unary_kernel_suffix();
...
void GroupConv2dDwSpatial::init_locals() {
...
void GroupConv2dDwSpatial::compute_grid_dims(uint32_t &x, uint32_t &y) {
...
void GroupConv2dDwSpatial::compute_mask(std::vector<uint32_t> &vmask) {
...
uint32_t GroupConv2dDwSpatial::get_lx_volume() {
    // h = p * stride_h - pad_h + r * dilation_h
    return ((m_split_stride - 1) * m_stride_h + (m_R - 1) * m_dilation_h + 1) * m_W * m_C;
}

std::string GroupConv2dDwSpatial::get_unary_kernel_suffix() {
    base::PostOp op = m_post_op.op();
    switch (op) {
...
uint32_t GroupConv2dDwSpatial::encode_unary_param0() {
    base::PostOp op = m_post_op.op();
    switch (op) {
    case base::PostOp::RELU:
        return 0;
...
bool GroupConv2dDwSpatial::is_unary_relu6() {
