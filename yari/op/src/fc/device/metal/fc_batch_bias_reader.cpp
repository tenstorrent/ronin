...
void read_px(Global gx, Local lzero, Pipe px, uint32 H, uint32 C,
...
void kernel(Global gx, Global gb, Global gzero, Local lzero, Pipe px, Pipe pb,
            uint32 N, uint32 H, uint32 C, uint32 K, uint32 zero_size,
  ...
  for (uint32 n = 0; n < N; n++) {
    ...
    for (uint32 h_start = 0; h_start < H; h_start += 32) {
      read_px(gx, lzero, px, H, C, x_start, h_start);
      x_start += C * 32;
    } // hw_start
    x_batch += x_stride;
  } // n
...
void kernel_main() {
  ...
  kernel(gx, gb, gzero, lzero, px, pb, N, H, C, K, zero_size, x_pos, x_stride);
}
