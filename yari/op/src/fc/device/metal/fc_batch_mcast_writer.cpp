...
void kernel_send(Global gw, Global gy, Pipe pw, Pipe py, Semaphore sem_send,
                 ...
                 uint32 x1, uint32 y1, uint32 num_dests, uint32 N, uint32 H,
                 uint32 C, uint32 K, uint32 y_pos, uint32 y_stride) {
  noc_semaphore_set(
...
void kernel(Global gw, Global gy, Pipe pw, Pipe py, Semaphore sem_send,
  ...
  if (send_mode != 0) {
    kernel_send(gw, gy, pw, py, sem_send, sem_recv, send_mode, x0, y0, x1, y1,
                num_dests, N, H, C, K, y_pos, y_stride);
  } else {
...
void kernel_main() {
  ...
  uint32 H = get_arg_val<uint32>(17);
  uint32 C = get_arg_val<uint32>(18);
  uint32 K = get_arg_val<uint32>(19);
