...
void kernel(Global gw, Global gy, Pipe pw, Pipe py, uint32 N, uint32 H,
...
void kernel_main() {
  ...
  uint32 H = get_arg_val<uint32>(9);
  uint32 C = get_arg_val<uint32>(10);
  uint32 K = get_arg_val<uint32>(11);
  uint32 y_pos = get_arg_val<uint32>(12);
  uint32 y_stride = get_arg_val<uint32>(13);
  kernel(gw, gy, pw, py, N, H, C, K, y_pos, y_stride);
}
