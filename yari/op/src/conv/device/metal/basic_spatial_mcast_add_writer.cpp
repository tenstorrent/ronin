...
void read_pw(Global gw, Pipe pw, Semaphore sem_send, Semaphore sem_recv,
             uint32 send_mode, uint32 x0, uint32 y0, uint32 x1, uint32 y1,
             uint32 num_dests, uint32 C, uint32 K, uint32 &w_start) {
  if (send_mode != 0) {
...
void kernel(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
            ...
            uint32 C, uint32 K, uint32 PQ, uint32 PQK_tail, uint32 RS,
  ...
  for (uint32 n = 0; n < N; n++) {
    for (uint32 pq_start = 0; pq_start < PQ; pq_start += 32) {
      ...
      for (uint32 rs = 0; rs < RS; rs++) {
        read_pw(gw, pw, sem_send, sem_recv, send_mode, x0, y0, x1, y1,
                num_dests, C, K, w_start);
      } // rs
      ...
      noc_async_read_global_dram(get_write_ptr(pz.cb_id) + (0 << 1), gz.addr,
                                 gz.log2_page_size, y_start << 1, count << 1);
      noc_async_read_barrier();
      ...
      noc_async_write_global_dram(get_read_ptr(py.cb_id) + (0 << 1), gy.addr,
                                  gy.log2_page_size, y_start << 1, count << 1);
...
void kernel_main() {
  ...
  uint32 PQK_tail = get_arg_val<uint32>(24);
  uint32 RS = get_arg_val<uint32>(25);
  uint32 y_pos = get_arg_val<uint32>(26);
  uint32 y_stride = get_arg_val<uint32>(27);
  kernel(gw, gz, gy, pw, pz, py, sem_send, sem_recv, send_mode, x0, y0, x1, y1,
         num_dests, N, C, K, PQ, PQK_tail, RS, y_pos, y_stride);
}
