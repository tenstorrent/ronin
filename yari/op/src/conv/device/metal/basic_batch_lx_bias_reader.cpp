...
void kernel(Global gx, Global gb, Global gzero, Global gmask, Local lx,
            ...
            uint32 K, uint32 R, uint32 S, uint32 HWC, uint32 PQ, uint32 start_p,
...
void kernel_main() {
  ...
  uint32 HWC = get_arg_val<uint32>(20);
  uint32 PQ = get_arg_val<uint32>(21);
  uint32 start_p = get_arg_val<uint32>(22);
  uint32 start_q = get_arg_val<uint32>(23);
  uint32 delta_p = get_arg_val<uint32>(24);
  uint32 delta_q = get_arg_val<uint32>(25);
  uint32 delta_r = get_arg_val<uint32>(26);
  uint32 delta_s = get_arg_val<uint32>(27);
  uint32 end_q = get_arg_val<uint32>(28);
  uint32 zero_size = get_arg_val<uint32>(29);
  uint32 mask_size = get_arg_val<uint32>(30);
  uint32 x_pos = get_arg_val<uint32>(31);
  uint32 x_stride = get_arg_val<uint32>(32);
  kernel(gx, gb, gzero, gmask, lx, lzero, lmask, px, pb, N, C, K, R, S, HWC, PQ,
         start_p, start_q, delta_p, delta_q, delta_r, delta_s, end_q, zero_size,
         mask_size, x_pos, x_stride);
}
