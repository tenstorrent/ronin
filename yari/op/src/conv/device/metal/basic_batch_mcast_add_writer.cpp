...
void kernel_send(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
                 Semaphore sem_send, Semaphore sem_recv, uint32 send_mode,
                 uint32 x0, uint32 y0, uint32 x1, uint32 y1, uint32 num_dests,
                 uint32 N, uint32 C, uint32 K, uint32 PQ, uint32 RS,
      ...
      for (uint32 rs = 0; rs < RS; rs++) {
        for (uint32 k_start = 0; k_start < K; k_start += 32) {
          ...
          noc_semaphore_set_multicast(
              sem_recv.addr,
              get_noc_multicast_addr(x0, y0, x1, y1, sem_recv.addr), num_dests);
...
void kernel_recv(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
                 Semaphore sem_send, Semaphore sem_recv, uint32 send_mode,
                 uint32 x0, uint32 y0, uint32 x1, uint32 y1, uint32 num_dests,
                 uint32 N, uint32 C, uint32 K, uint32 PQ, uint32 RS,
                 uint32 y_pos, uint32 y_stride) {
...
void kernel(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
            ...
            uint32 C, uint32 K, uint32 PQ, uint32 RS, uint32 y_pos,
            uint32 y_stride) {
  if (send_mode != 0) {
    kernel_send(gw, gz, gy, pw, pz, py, sem_send, sem_recv, send_mode, x0, y0,
                x1, y1, num_dests, N, C, K, PQ, RS, y_pos, y_stride);
  } else {
    kernel_recv(gw, gz, gy, pw, pz, py, sem_send, sem_recv, send_mode, x0, y0,
                x1, y1, num_dests, N, C, K, PQ, RS, y_pos, y_stride);
  }
}
...
void kernel_main() {
  ...
  uint32 C = get_arg_val<uint32>(21);
  uint32 K = get_arg_val<uint32>(22);
  uint32 PQ = get_arg_val<uint32>(23);
  uint32 RS = get_arg_val<uint32>(24);
  uint32 y_pos = get_arg_val<uint32>(25);
  uint32 y_stride = get_arg_val<uint32>(26);
  kernel(gw, gz, gy, pw, pz, py, sem_send, sem_recv, send_mode, x0, y0, x1, y1,
         num_dests, N, C, K, PQ, RS, y_pos, y_stride);
}
