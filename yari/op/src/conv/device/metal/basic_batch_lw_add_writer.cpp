...
void kernel(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
            uint32 N, uint32 K, uint32 PQ, uint32 RSKC, uint32 y_pos,
            uint32 y_stride) {
...
void kernel_main() {
  ...
  uint32 RSKC = get_arg_val<uint32>(15);
  uint32 y_pos = get_arg_val<uint32>(16);
  uint32 y_stride = get_arg_val<uint32>(17);
  kernel(gw, gz, gy, pw, pz, py, N, K, PQ, RSKC, y_pos, y_stride);
}
