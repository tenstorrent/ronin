...
void read_px(Global gx, Local lzero, Pipe px, uint32 HW, uint32 C,
...
void kernel(Global gx, Global gb, Global gzero, Local lzero, Pipe px, Pipe pb,
            uint32 N, uint32 HW, uint32 C, uint32 K, uint32 zero_size,
  ...
  for (uint32 n = 0; n < N; n++) {
    ...
    for (uint32 hw_start = 0; hw_start < HW; hw_start += 32) {
      read_px(gx, lzero, px, HW, C, x_start, hw_start);
      x_start += C * 32;
    } // hw_start
    x_batch += x_stride;
  } // n
...
void kernel_main() {
  ...
  uint32 zero_size = get_arg_val<uint32>(15);
  uint32 x_pos = get_arg_val<uint32>(16);
  uint32 x_stride = get_arg_val<uint32>(17);
  kernel(gx, gb, gzero, lzero, px, pb, N, HW, C, K, zero_size, x_pos, x_stride);
}
