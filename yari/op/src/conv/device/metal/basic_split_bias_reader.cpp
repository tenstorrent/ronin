...
void kernel_send(Global gx, Global gb, Global gzero, Global gmask, Local lzero,
                 Local lmask, Pipe px, Pipe pb, Semaphore sem_send,
                 Semaphore sem_recv, uint32 send_mode, uint32 x0, uint32 y0,
                 uint32 x1, uint32 y1, uint32 num_dests, uint32 N, uint32 C,
                 uint32 R, uint32 S, uint32 PQ, uint32 Kb, uint32 start_p,
                 uint32 start_q, uint32 delta_p, uint32 delta_q, uint32 delta_r,
                 uint32 delta_s, uint32 end_q, uint32 zero_size,
  ...
  noc_async_read_global_dram(get_write_ptr(pb.cb_id) + (0 << 1), gb.addr,
                             gb.log2_page_size, b_pos << 1, (Kb * 32) << 1);
  noc_async_read_barrier();
      ...
      for (uint32 r = 0; r < R; r++) {
        ...
        for (uint32 s = 0; s < S; s++) {
          ...
          noc_async_write_multicast(
              get_write_ptr(px.cb_id) + (0 << 1),
              get_noc_multicast_addr(x1, y1, x0, y0,
                                     get_write_ptr(px.cb_id) + (0 << 1)),
              (C * 32) << 1, num_dests);
          noc_semaphore_set_multicast(
              sem_recv.addr,
              get_noc_multicast_addr(x1, y1, x0, y0, sem_recv.addr), num_dests);
...
void kernel_recv(Global gx, Global gb, Global gzero, Global gmask, Local lzero,
                 Local lmask, Pipe px, Pipe pb, Semaphore sem_send,
                 Semaphore sem_recv, uint32 send_mode, uint32 x0, uint32 y0,
                 uint32 x1, uint32 y1, uint32 num_dests, uint32 N, uint32 C,
                 uint32 R, uint32 S, uint32 PQ, uint32 Kb, uint32 start_p,
    ...
    for (uint32 pq_start = 0; pq_start < PQ; pq_start += 32) {
      for (uint32 r = 0; r < R; r++) {
        for (uint32 s = 0; s < S; s++) {
          cb_reserve_back(px.cb_id, px.frame_size);
          noc_semaphore_set(
              reinterpret_cast<volatile tt_l1_ptr uint32_t *>(sem_recv.addr),
              0);
...
void kernel(Global gx, Global gb, Global gzero, Global gmask, Local lzero,
            ...
            uint32 R, uint32 S, uint32 PQ, uint32 Kb, uint32 start_p,
            uint32 start_q, uint32 delta_p, uint32 delta_q, uint32 delta_r,
            uint32 delta_s, uint32 end_q, uint32 zero_size, uint32 mask_size,
            uint32 x_pos, uint32 x_stride, uint32 b_pos) {
  if (send_mode != 0) {
    kernel_send(gx, gb, gzero, gmask, lzero, lmask, px, pb, sem_send, sem_recv,
                send_mode, x0, y0, x1, y1, num_dests, N, C, R, S, PQ, Kb,
                start_p, start_q, delta_p, delta_q, delta_r, delta_s, end_q,
                zero_size, mask_size, x_pos, x_stride, b_pos);
  } else {
    kernel_recv(gx, gb, gzero, gmask, lzero, lmask, px, pb, sem_send, sem_recv,
                send_mode, x0, y0, x1, y1, num_dests, N, C, R, S, PQ, Kb,
                start_p, start_q, delta_p, delta_q, delta_r, delta_s, end_q,
                zero_size, mask_size, x_pos, x_stride, b_pos);
  }
...
void kernel_main() {
  ...
  Semaphore sem_send;
  sem_send.addr = tanto_get_semaphore(get_arg_val<uint32>(14));
  Semaphore sem_recv;
  sem_recv.addr = tanto_get_semaphore(get_arg_val<uint32>(15));
  uint32 send_mode = get_arg_val<uint32>(16);
  uint32 x0 = get_arg_val<uint32>(17);
  uint32 y0 = get_arg_val<uint32>(18);
  uint32 x1 = get_arg_val<uint32>(19);
  uint32 y1 = get_arg_val<uint32>(20);
  uint32 num_dests = get_arg_val<uint32>(21);
  uint32 N = get_arg_val<uint32>(22);
  uint32 C = get_arg_val<uint32>(23);
  uint32 R = get_arg_val<uint32>(24);
  uint32 S = get_arg_val<uint32>(25);
  uint32 PQ = get_arg_val<uint32>(26);
  uint32 Kb = get_arg_val<uint32>(27);
  uint32 start_p = get_arg_val<uint32>(28);
  uint32 start_q = get_arg_val<uint32>(29);
  uint32 delta_p = get_arg_val<uint32>(30);
  uint32 delta_q = get_arg_val<uint32>(31);
  uint32 delta_r = get_arg_val<uint32>(32);
  uint32 delta_s = get_arg_val<uint32>(33);
  uint32 end_q = get_arg_val<uint32>(34);
  uint32 zero_size = get_arg_val<uint32>(35);
  uint32 mask_size = get_arg_val<uint32>(36);
  uint32 x_pos = get_arg_val<uint32>(37);
  uint32 x_stride = get_arg_val<uint32>(38);
  uint32 b_pos = get_arg_val<uint32>(39);
  kernel(gx, gb, gzero, gmask, lzero, lmask, px, pb, sem_send, sem_recv,
         send_mode, x0, y0, x1, y1, num_dests, N, C, R, S, PQ, Kb, start_p,
         start_q, delta_p, delta_q, delta_r, delta_s, end_q, zero_size,
         mask_size, x_pos, x_stride, b_pos);
}
