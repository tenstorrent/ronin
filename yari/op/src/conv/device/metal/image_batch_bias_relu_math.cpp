...
void kernel(Pipe px, Pipe pw, Pipe pb, Pipe py, Pipe px_im, Pipe py_im,
            uint32 N, uint32 K, uint32 Ko, uint32 Ki, uint32 PQ, uint32 RSC_rnd,
            uint32 unary_param0) {
  tanto_relu_init();
  ...
  for (uint32 n = 0; n < N; n++) {
    for (uint32 pq_start = 0; pq_start < PQ; pq_start += 32) {
      ...
      tanto_unpack_tilize_block_init(px.cb_id, RSCt);
      tanto_copy_init();
      tanto_pack_init(px_im.cb_id);
      tilize_block(px.cb_id, RSCt, px_im.cb_id);
      ...
      for (uint32 ko = 0; ko < Ko; ko++) {
        ...
        for (uint32 ki = 0; ki < Ki; ki++) {
          matmul_slice(px_im, pw, w_start, ki, RSCt);
          w_start += RSCt;
        }
...
void MAIN {
  ...
  uint32 RSC_rnd = get_arg_val<uint32>(17);
  uint32 unary_param0 = get_arg_val<uint32>(18);
  tanto_compute_init();
  kernel(px, pw, pb, py, px_im, py_im, N, K, Ko, Ki, PQ, RSC_rnd, unary_param0);
}
} // namespace NAMESPACE
