...
void kernel(Global gw, Global gy, Pipe pw, Pipe py, Semaphore sem_send,
            ...
            uint32 PQ, uint32 RS, uint32 KC, uint32 Kb, uint32 KbC,
...
void kernel_main() {
  ...
  uint32 KC = get_arg_val<uint32>(20);
  uint32 Kb = get_arg_val<uint32>(21);
  uint32 KbC = get_arg_val<uint32>(22);
  uint32 RSKbC = get_arg_val<uint32>(23);
  uint32 w_pos = get_arg_val<uint32>(24);
