...
void kernel(Global gw, Global gz, Global gy, Pipe pw, Pipe pz, Pipe py,
            ...
            uint32 K, uint32 PQ, uint32 RSKC, uint32 y_pos, uint32 y_stride) {
...
void kernel_main() {
  ...
  uint32 y_pos = get_arg_val<uint32>(24);
  uint32 y_stride = get_arg_val<uint32>(25);
  kernel(gw, gz, gy, pw, pz, py, sem_send, sem_recv, send_mode, x0, y0, x1, y1,
         num_dests, N, K, PQ, RSKC, y_pos, y_stride);
}
