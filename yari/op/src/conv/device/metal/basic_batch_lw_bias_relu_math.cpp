...
void kernel(Pipe px, Pipe pw, Pipe pb, Pipe py, Pipe px_im, Pipe py_im,
            ...
            uint32 RS, uint32 RSKC, uint32 unary_param0) {
  tanto_relu_init();
  uint32 Ct = C / 32;
  uint32 Kt = K / 32;
  px.frame_size = Ct;
...
void MAIN {
  ...
  uint32 RSKC = get_arg_val<uint32>(19);
  uint32 unary_param0 = get_arg_val<uint32>(20);
  tanto_compute_init();
  kernel(px, pw, pb, py, px_im, py_im, N, C, K, Ko, Ki, PQ, RS, RSKC,
         unary_param0);
}
} // namespace NAMESPACE
