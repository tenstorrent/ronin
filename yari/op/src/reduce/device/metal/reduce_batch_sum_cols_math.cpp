...
void reduce_op(Pipe px, Pipe ps) {
  ...
  reduce_tile<PoolType::SUM, ReduceDim::REDUCE_COL>(px.cb_id, ps.cb_id, 0, 0,
                                                    0);
}
...
void kernel(Pipe px, Pipe ps, Pipe py, Pipe px_im, Pipe py_im, uint32 N,
  ...
  for (uint32 n = 0; n < N; n++) {
    for (uint32 h = 0; h < Ht; h++) {
      ...
      px_im.frame_size = 1;
      py_im.frame_size = 1;
      tanto_pack_col_init(py_im.cb_id);
    ...
    untilize_block<1>(py_im.cb_id, Wt, py.cb_id);
    cb_pop_front(py_im.cb_id, py_im.frame_size);
