...
void kernel(Global gy, Pipe py, uint32 N, uint32 W, uint32 y_pos,
            uint32 y_stride) {
  py.frame_size = (W / 32);
  ...
  for (uint32 n = 0; n < N; n++) {
    ...
    noc_async_write_global_dram(get_read_ptr(py.cb_id) + (0 << 1), gy.addr,
                                gy.log2_page_size, y_start << 1, (W * 32) << 1);
