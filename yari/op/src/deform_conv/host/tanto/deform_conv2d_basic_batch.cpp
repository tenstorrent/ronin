// SPDX-FileCopyrightText: © 2025 Tenstorrent AI ULC
//
// SPDX-License-Identifier: Apache-2.0

#include <cstdint>
#include <cassert>
#include <string>
#include <vector>

#include "host/core/api.hpp"

#include "host/base/post_op.hpp"

#include "host/util/transform.hpp"

#include "host/tanto/util.hpp"
#include "host/tanto/deform_conv2d_basic_batch.hpp"

namespace ronin {
namespace op {
namespace deform_conv {
namespace tanto {

namespace core = ronin::tanto::host;
namespace base = ronin::op::common::base;
namespace util = ronin::op::common::util;

constexpr bool ENABLE_CACHE_LX = true; // reserved
constexpr bool ENABLE_CACHE_LW = true; // reserved

//
//    DeformConv2dBasicBatch
//

DeformConv2dBasicBatch::DeformConv2dBasicBatch(
        int N,
        int H,
        int W,
        int C,
        int P,
        int Q,
        int K,
        int R,
        int S,
        int pad_h,
        int pad_w,
        int stride_h,
        int stride_w,
        int dilation_h,
        int dilation_w,
        int deform_groups,
        const base::PostOpSpec &post_op,
        int batch_size):
            m_N(uint32_t(N)),
            m_H(uint32_t(H)),
            m_W(uint32_t(W)),
            m_C(uint32_t(C)),
            m_P(uint32_t(P)),
            m_Q(uint32_t(Q)),
            m_K(uint32_t(K)),
            m_R(uint32_t(R)),
            m_S(uint32_t(S)),
            m_pad_h(uint32_t(pad_h)),
            m_pad_w(uint32_t(pad_w)),
            m_stride_h(uint32_t(stride_h)),
            m_stride_w(uint32_t(stride_w)),
            m_dilation_h(uint32_t(dilation_h)),
            m_dilation_w(uint32_t(dilation_w)),
            m_deform_groups(uint32_t(deform_groups)),
            m_post_op(post_op),
            m_batch_size(uint32_t(batch_size)) { 
    // these values are required to compute input/output volumes and shapes
    m_C_arg = m_C;
    m_K_arg = m_K;
    m_C = u32_align(m_C, 32);
    m_K = u32_align(m_K, 32);
    m_D_arg = m_deform_groups * m_R * m_S * 2;
    m_D = u32_align(m_D_arg, 32);
}

DeformConv2dBasicBatch::~DeformConv2dBasicBatch() { }

void DeformConv2dBasicBatch::init(
        const core::Device &device,
        const core::Global &gx,
        const core::Global &gd,
        const core::Global &gw,
        const core::Global &gb,
        const core::Global &gz,
        const core::Global &gy) {
    m_device = device;
    m_gx = gx;
    m_gd = gd;
    m_gw = gw;
    m_gb = gb;
    m_gz = gz;
    m_gy = gy;

    assert(m_batch_size < 8 || m_batch_size % 8 == 0);
    // ACHTUNG: Temporary limit 64 is Wormhole-specific
    assert(m_batch_size <= 64);
    assert(m_N % m_batch_size == 0);

    assert(m_C % 32 == 0);
    assert(m_K % 32 == 0);

    // deform groups are not supported
    assert(m_deform_groups == 1);

    init_config();

    m_program = core::Program(m_device);

    uint32_t grid_x, grid_y;
    compute_grid_dims(grid_x, grid_y);

    m_x_start = 0;
    m_y_start = 0;
    m_x_end = grid_x - 1;
    m_y_end = grid_y - 1;

    m_grid = core::Grid(m_program, m_x_start, m_y_start, m_x_end, m_y_end);

    m_zero_size = m_C;

    uint32_t Ct = m_C / 32;
    split_chan(Ct, m_Co, m_Ci);
    uint32_t Kt = m_K / 32;
    split_chan(Kt, m_Ko, m_Ki);
    uint32_t Dt = m_D / 32;

    m_px_frame_size = Ct;
    m_pd_frame_size = Dt;
    m_pw_frame_size = Ct;
    if (!m_gb.is_null()) {
        m_pb_frame_size = Kt;
    }
    if (!m_gz.is_null()) {
        m_pz_frame_size = Kt;
    }
    m_py_frame_size = Kt;
    m_px_im_frame_size = Ct;
    m_pt_im_frame_size = Ct;
    m_pz_im_frame_size = Kt;
    m_py_im_frame_size = Kt;
    m_pd1_im_frame_size = Dt;
    m_pd2_im_frame_size = Dt;
    m_ppi_im_frame_size = Dt;
    m_ppc_im_frame_size = Dt * 2;
    m_pc1_im_frame_size = 4;
    m_pc2_im_frame_size = 4;

    m_start_p = uint32_t(-int32_t(m_pad_h));
    m_start_q = uint32_t(-int32_t(m_pad_w));
    m_delta_p = m_stride_h;
    m_delta_q = m_stride_w;
    m_delta_r = m_dilation_h;
    m_delta_s = m_dilation_w;
    m_end_q = m_start_q + m_Q * m_delta_q;

    m_kernel_base_path = "op/deform_conv/device/metal";
    m_defines = {{"T", "bfloat16"}};

    init_options();
    validate_globals();

    create_globals();
    create_locals();
    create_pipes();
    create_semaphores();
    create_kernels();

    init_locals();
}

void DeformConv2dBasicBatch::run() {
    core::Queue queue(m_device, 0);
    queue.enqueue_program(m_program, false);
}

int DeformConv2dBasicBatch::input_volume(int index) {
    switch (index) {
    case 0:
        return m_N * u32_align(m_H * m_W, 32) * m_C;
    case 1:
        return m_N * u32_align(m_P * m_Q, 32) * m_D;
    case 2:
        return m_R * m_S * m_K * m_C;
    case 3:
        return m_K * 32;
    case 4:
        return m_N * u32_align(m_P * m_Q, 32) * m_K;
    default:
        assert(false);
        return 0;
    }
}

int DeformConv2dBasicBatch::output_volume(int index) {
    assert(index == 0);
    return m_N * u32_align(m_P * m_Q, 32) * m_K;
}

std::vector<float> DeformConv2dBasicBatch::
        transform_input(int index, const std::vector<float> &x) {
    std::vector<float> y;
    switch (index) {
    case 0:
        y = util::pad(x, m_N, m_H * m_W, m_C_arg, m_N, u32_align(m_H * m_W, 32), m_C);
        break;
    case 1:
        y = util::pad(x, m_N, m_P * m_Q, m_D_arg, m_N, u32_align(m_P * m_Q, 32), m_D);
        break;
    case 2:
        y = util::pad(x, m_R * m_S, m_K_arg, m_C_arg, m_R * m_S, m_K, m_C);
        y = util::tilize(y, m_R * m_S * m_K, m_C);
        y = util::make_faces(y);
        break;
    case 3:
        y = util::pad(x, 1, m_K_arg, 32, m_K);
        y = util::tilize(y, 32, m_K);
        y = util::make_faces(y);
        break;
    case 4:
        y = util::pad(x, m_N, m_P * m_Q, m_K_arg, m_N, u32_align(m_P * m_Q, 32), m_K);
        break;
    default:
        assert(false);
        break;
    }
    return y;
}

std::vector<float> DeformConv2dBasicBatch::
        transform_output(int index, const std::vector<float> &x) {
    assert(index == 0);
    return util::unpad(x, m_N, u32_align(m_P * m_Q, 32), m_K, m_N, m_P * m_Q, m_K_arg);
}

void DeformConv2dBasicBatch::init_config() {
    m_cache_lx = false; // reserved
    m_cache_lw = false; // reserved
}

void DeformConv2dBasicBatch::init_options() {
    m_options = 0;
    if (!m_gb.is_null()) {
        m_options |= OPT_BIAS;
    }
    if (!m_gz.is_null()) {
        m_options |= OPT_ADD;
    }
    base::PostOp op = m_post_op.op();
    if (op != base::PostOp::NONE) {
        m_options |= OPT_UNARY;
    }
}

void DeformConv2dBasicBatch::validate_globals() {
    uint32_t item_bytes = get_item_bytes(T);
    assert(!m_gx.is_null());
    assert(!m_gd.is_null());
    assert(!m_gw.is_null());
    assert(!m_gy.is_null());
    assert(m_gx.bytes() >= input_volume(0) * item_bytes);
    assert(m_gd.bytes() >= input_volume(1) * item_bytes);
    assert(m_gw.bytes() == input_volume(2) * item_bytes);
    if (!m_gb.is_null()) {
        assert(m_gb.bytes() == input_volume(3) * item_bytes);
    }
    if (!m_gz.is_null()) {
        assert(m_gz.bytes() >= input_volume(4) * item_bytes);
    }
    assert(m_gy.bytes() >= output_volume(0) * item_bytes);
}

void DeformConv2dBasicBatch::create_globals() {
    uint32_t log2_page_size = 10; // 2 ^ 10 = 1024 items
    m_gzero = core::Global(m_device, T, m_zero_size, log2_page_size);
}

void DeformConv2dBasicBatch::create_locals() {
    m_lzero = core::Local(m_program, m_grid, T, m_zero_size);
}

void DeformConv2dBasicBatch::create_pipes() {
    m_px =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INPUT,
            T,
            m_px_frame_size * 2,
            m_px_frame_size);
    m_pd =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INPUT,
            T,
            m_pd_frame_size * 2,
            m_pd_frame_size);
    m_pw =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INPUT,
            T,
            m_pw_frame_size * 2,
            m_pw_frame_size);
    if (!m_gb.is_null()) {
        m_pb =
            core::Pipe(
                m_program,
                m_grid,
                core::PipeKind::INPUT,
                T,
                m_pb_frame_size * 2,
                m_pb_frame_size);
    }
#if 0 // reserved
    if (!m_gz.is_null()) {
        m_pz =
            core::Pipe(
                m_program,
                m_grid,
                core::PipeKind::INPUT,
                T,
                m_pz_frame_size * 2,
                m_pz_frame_size);
    }
#endif
    m_py =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::OUTPUT,
            T,
            m_py_frame_size * 2,
            m_py_frame_size);
    m_px_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_px_im_frame_size * 2,
            m_px_im_frame_size);
    m_pt_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_pt_im_frame_size * 2,
            m_pt_im_frame_size);
#if 0 // reserved
    if (!m_gz.is_null()) {
        m_pz_im =
            core::Pipe(
                m_program,
                m_grid,
                core::PipeKind::INTERMED,
                T,
                m_pz_im_frame_size * 2,
                m_pz_im_frame_size);
    }
#endif
    m_py_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_py_im_frame_size * 2,
            m_py_im_frame_size);

    m_pd1_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_pd1_im_frame_size * 2,
            m_pd1_im_frame_size);
    m_pd2_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_pd2_im_frame_size * 2,
            m_pd2_im_frame_size);
    // output pipe: compute -> dataflow
    m_ppi_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::OUTPUT,
            core::DataFormat::UINT16,
            m_ppi_im_frame_size * 2,
            m_ppi_im_frame_size);
    // output pipe: compute -> dataflow
    m_ppc_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::OUTPUT,
            T,
            m_ppc_im_frame_size * 2,
            m_ppc_im_frame_size);
    // input pipe: dataflow -> compute
    m_pc1_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INPUT,
            T,
            m_pc1_im_frame_size * 2,
            m_pc1_im_frame_size);
    m_pc2_im =
        core::Pipe(
            m_program,
            m_grid,
            core::PipeKind::INTERMED,
            T,
            m_pc2_im_frame_size * 2,
            m_pc2_im_frame_size);
}

void DeformConv2dBasicBatch::create_semaphores() {
    m_sem_send = core::Semaphore(m_program, m_grid, 0);
    m_sem_recv = core::Semaphore(m_program, m_grid, 0);
}

void DeformConv2dBasicBatch::create_kernels() {
    uint32_t reader_options = m_options & OPT_BIAS;
    uint32_t writer_options = m_options & OPT_ADD;
    uint32_t math_options = m_options;
    if (reader_options == OPT_BIAS) {
        create_bias_reader();
    } else {
        // not yet implemented
        assert(false);
    }
    if (writer_options == OPT_ADD) {
        // not yet implemented
        assert(false);
    } else {
        create_mcast_writer();
    }
    if (math_options == OPT_BIAS) {
        create_bias_math();
    } else {
        // not yet implemented
        assert(false);
    }
}

void DeformConv2dBasicBatch::create_bias_reader() {
    std::string path = m_kernel_base_path + "/basic_batch_bias_reader.cpp";
    m_reader = 
        core::Kernel(
            m_program, 
            m_grid, 
            core::KernelKind::READER, 
            core::KernelFormat::METAL,
            path, 
            {}, 
            m_defines);
/*
void kernel(
        global<T> gx,
        global<T> gd,
        global<T> gb,
        global<T> gzero,
        local<T> lzero,
        pipe<T> px,
        pipe<T> pd,
        pipe<T> pb,
        pipe<uint16> ppi_im,
        pipe<T> ppc_im,
        pipe<T> pc1_im,
        uint32 N,
        uint32 H,
        uint32 W,
        uint32 C,
        uint32 K,
        uint32 R,
        uint32 S,
        uint32 D,
        uint32 PQ,
        uint32 start_p,
        uint32 start_q,
        uint32 delta_p,
        uint32 delta_q,
        uint32 delta_r,
        uint32 delta_s,
        uint32 end_q,
        uint32 zero_size,
        uint32 x_pos,
        uint32 x_stride,
        uint32 d_pos,
        uint32 d_stride)
*/
    uint32_t HW_rnd = u32_align(m_H * m_W, 32);
    uint32_t x_stride = m_batch_size * HW_rnd * m_C;
    uint32_t PQ_rnd = u32_align(m_P * m_Q, 32);
    uint32_t d_stride = m_batch_size * PQ_rnd * m_D;
    std::vector<core::KernelArg> args{
        m_gx,
        m_gd,
        m_gb,
        m_gzero,
        m_lzero,
        m_px,
        m_pd,
        m_pb,
        m_ppi_im,
        m_ppc_im,
        m_pc1_im,
        m_N / m_batch_size,
        m_H,
        m_W,
        m_C,
        m_K,
        m_R,
        m_S,
        m_D,
        m_P * m_Q,
        m_start_p,
        m_start_q,
        m_delta_p,
        m_delta_q,
        m_delta_r,
        m_delta_s,
        m_end_q,
        m_zero_size,
        uint32_t(0), // [28] x_pos
        x_stride,
        uint32_t(0), // [30] d_pos
        d_stride
    };
    uint32_t x_inc = HW_rnd * m_C;
    uint32_t x_pos = 0;
    uint32_t d_inc = PQ_rnd * m_D;
    uint32_t d_pos = 0;
    for (uint32_t x = m_x_start; x <= m_x_end; x++) {
        for (uint32_t y = m_y_start; y <= m_y_end; y++) {
            args[28] = x_pos;
            args[30] = d_pos;
            m_reader.set_args(x, y, args);
            x_pos += x_inc;
            d_pos += d_inc;
        }
    }
}

void DeformConv2dBasicBatch::create_mcast_writer() {
    std::string path = m_kernel_base_path + "/basic_batch_mcast_writer.cpp";
    m_writer = 
        core::Kernel(
            m_program, 
            m_grid, 
            core::KernelKind::WRITER, 
            core::KernelFormat::METAL,
            path, 
            {}, 
            m_defines);
/*
void kernel(
        global<T> gw,
        global<T> gy,
        pipe<T> pw,
        pipe<T> py,
        semaphore sem_send,
        semaphore sem_recv,
        uint32 send_mode,
        uint32 x0,
        uint32 y0,
        uint32 x1,
        uint32 y1,
        uint32 num_dests,
        uint32 N,
        uint32 C,
        uint32 K,
        uint32 PQ,
        uint32 RS,
        uint32 y_pos,
        uint32 y_stride)
*/
    uint32_t num_dests = m_batch_size / 8 - 1; // Wormhole-specific, temporary
    uint32_t PQ_rnd = u32_align(m_P * m_Q, 32);
    uint32_t y_stride = (m_batch_size - 1) * PQ_rnd * m_K;
    std::vector<core::KernelArg> args{
        m_gw,
        m_gy,
        m_pw,
        m_py,
        m_sem_send,
        m_sem_recv,
        uint32_t(0), // [6] send_mode
        uint32_t(0), // [7] x0
        uint32_t(0), // [8] y0
        uint32_t(0), // [9] x1
        uint32_t(0), // [10] y1
        num_dests,
        m_N / m_batch_size,
        m_C,
        m_K,
        m_P * m_Q,
        m_R * m_S,
        uint32_t(0), // [17] y_pos
        y_stride
    };
    uint32_t y_inc = PQ_rnd * m_K;
    uint32_t y_pos = 0;
    for (uint32_t x = m_x_start; x <= m_x_end; x++) {
        for (uint32_t y = m_y_start; y <= m_y_end; y++) {
            uint32_t send_mode = (y == m_y_start) ? 1 : 0;
            uint32_t x0_phy, y0_phy;
            m_device.worker_core_from_logical_core(x, m_y_start, x0_phy, y0_phy);
            uint32_t x1_phy, y1_phy;
            m_device.worker_core_from_logical_core(x, m_y_end, x1_phy, y1_phy);
            args[6] = send_mode;
            args[7] = x0_phy;
            args[8] = y0_phy;
            args[9] = x1_phy;
            args[10] = y1_phy;
            args[17] = y_pos;
            m_writer.set_args(x, y, args);
            y_pos += y_inc;
        }
    }
}

void DeformConv2dBasicBatch::create_bias_math() {
    std::string path = m_kernel_base_path + "/basic_batch_bias_math.cpp";
    m_math = 
        core::Kernel(
            m_program, 
            m_grid, 
            core::KernelKind::MATH, 
            core::KernelFormat::METAL,
            path, 
            {}, 
            m_defines);
/*
void kernel(
        pipe<T> px,
        pipe<T> pd,
        pipe<T> pw,
        pipe<T> pb,
        pipe<T> py,
        pipe<T> px_im,
        pipe<T> pt_im,
        pipe<T> py_im,
        pipe<T> pd1_im,
        pipe<T> pd2_im,
        pipe<uint16> ppi_im,
        pipe<T> ppc_im,
        pipe<T> pc1_im,
        pipe<T> pc2_im,
        uint32 N,
        uint32 C,
        uint32 K,
        uint32 D,
        uint32 Co,
        uint32 Ci,
        uint32 Ko,
        uint32 Ki,
        uint32 PQ,
        uint32 RS)
*/
    std::vector<core::KernelArg> args{
        m_px,
        m_pd,
        m_pw,
        m_pb,
        m_py,
        m_px_im,
        m_pt_im,
        m_py_im,
        m_pd1_im,
        m_pd2_im,
        m_ppi_im,
        m_ppc_im,
        m_pc1_im,
        m_pc2_im,
        m_N / m_batch_size,
        m_C,
        m_K,
        m_D,
        m_Co,
        m_Ci,
        m_Ko,
        m_Ki,
        m_P * m_Q,
        m_R * m_S
    };
    m_math.set_args(m_grid, args);
}

void DeformConv2dBasicBatch::init_locals() {
    std::vector<uint32_t> vzero(m_gzero.bytes() / sizeof(uint32_t), 0);
    core::Queue queue(m_device, 0);
    queue.enqueue_write(m_gzero, vzero.data(), true);
}

void DeformConv2dBasicBatch::compute_grid_dims(uint32_t &x, uint32_t &y) {
    // ACHTUNG: Temporary limit 8 is Wormhole-specific
    if (m_batch_size <= 8) {
        x = m_batch_size;
        y = 1;
    } else {
        x = 8;
        y = m_batch_size / 8;
    }
}

std::string DeformConv2dBasicBatch::get_unary_kernel_suffix() {
    base::PostOp op = m_post_op.op();
    switch (op) {
    case base::PostOp::RELU:
        return "relu";
    case base::PostOp::CLIP:
        if (is_unary_relu6()) {
            return "relu6";
        }
        // generic clip is not yet implemented
        assert(false);
        return "<?>";
    default:
        assert(false);
        return "<?>";
    }
}

uint32_t DeformConv2dBasicBatch::get_unary_op_code() {
    // these op codes must be used by all kernels
    static constexpr uint32_t
        UNARY_OP_RELU = 0,
        UNARY_OP_RELU6 = 1;
    base::PostOp op = m_post_op.op();
    switch (op) {
    case base::PostOp::RELU:
        return UNARY_OP_RELU;
    case base::PostOp::CLIP:
        if (is_unary_relu6()) {
            return UNARY_OP_RELU6;
        }
        // generic clip is not yet implemented
        assert(false);
        return 0;
    default:
        assert(false);
        return 0;
    }
}

uint32_t DeformConv2dBasicBatch::encode_unary_param0() {
    base::PostOp op = m_post_op.op();
    switch (op) {
    case base::PostOp::RELU:
        return 0;
    case base::PostOp::CLIP:
        if (is_unary_relu6()) {
            // param0 hardcoded in kernels
            return 0;
        }
        // generic clip is not yet implemented
        assert(false);
        return 0;
    default:
        assert(false);
        return 0;
    }
}

bool DeformConv2dBasicBatch::is_unary_relu6() {
    return (m_post_op.alpha() == 0.0f && m_post_op.beta() == 6.0f);
}

void DeformConv2dBasicBatch::split_chan(uint32_t ct, uint32_t &co, uint32_t &ci) {
    if (ct <= DEST_TILES) {
        ci = ct;
    } else if (ct % DEST_TILES == 0) {
        ci = DEST_TILES;
    } else {
        // TODO: Implement better rule?
        ci = 1;
    }
    co = ct / ci;
    assert(ci * co == ct);
}

} // namespace tanto
} // namespace deform_conv
} // namespace op
} // namespace ronin

