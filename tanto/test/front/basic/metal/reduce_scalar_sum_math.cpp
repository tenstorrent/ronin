...
void reduce_op(Pipe px, Pipe ps) {
  ...
  reduce_tile<PoolType::SUM, ReduceDim::REDUCE_SCALAR>(px.cb_id, ps.cb_id, 0, 0,
                                                       0);
}

void kernel(Pipe px, Pipe ps, Pipe py, uint32 Ht, uint32 Wt, uint32 NC) {
  tanto_unpack_reduce_scalar_init(px.cb_id, ps.cb_id);
  tanto_reduce_sum_scalar_init();
  tanto_pack_scalar_init(py.cb_id);
