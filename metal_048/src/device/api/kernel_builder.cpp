...
void KernelBuilderImpl::build(
    ...
    std::string cpp_cmd = make_cpp_cmd(name, compile_time_args, defines, obj_path);
    run_cpp_cmd(cpp_cmd);
    if (is_compute) {
...
std::string KernelBuilderImpl::make_cpp_cmd(
    ...
    std::string cmd = m_cpp_cmd_base + " -o " + obj_path;
    int compile_time_arg_count = int(compile_time_args.size());
    for (int i = 0; i < compile_time_arg_count; i++) {
        std::string name = make_compile_time_arg_name(i);
        uint32_t value = compile_time_args[i];
        cmd += " -D" + name + "=" + std::to_string(value);
    }
    int define_count = int(defines.size());
    for (int i = 0; i < define_count; i++) {
        std::string name = defines[i].first;
        std::string value = defines[i].second;
        if (!value.empty()) {
            cmd += " -D" + name + "=" + quote_define_value(value);
        } else {
            cmd += " -D" + name;
        }
    }
    cmd += " " + m_src_base_dir + file_name;
    return cmd;
}
#endif
