...
void mm_init_once(Compute *api, Riscv32Core *core) {
    api->mm_init_once();
}
#endif
...
#if 0 // TODO: Revise this
void copy_tile_matmul_partials_init_short_with_dt(Compute *api, Riscv32Core *core) {
    uint32_t cbid = core->get_arg(0);
    api->copy_tile_matmul_partials_init_short_with_dt(cbid);
}
#endif
...
#if 0 // SKIPPED
void graph_interpreter_init(Compute *api, Riscv32Core *core) { 
    api->graph_interpreter_init();
}

void get_next_op_info(Compute *api, Riscv32Core *core) {
    // not supported
    op_info_t *op_info = ...
    api->get_next_op_info(*op_info);
}
#endif
