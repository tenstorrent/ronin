...
TensixImpl::TensixImpl(
    ...
    uint32_t my_x = soc_arch->worker_logical_to_routing_x(logical_x);
    uint32_t my_y = soc_arch->worker_logical_to_routing_y(logical_y);
    m_noc.reset(new NocImpl(soc, noc_arch, my_x, my_y));
...
void TensixImpl::setup_cb() {
    uint32_t *ptr = 
        reinterpret_cast<uint32_t *>(
            m_l1->map_addr(AddrMap::CIRCULAR_BUFFER_CONFIG_BASE));
...
void TensixImpl::clear_code() {
    ...
    MemMap *mem_map = m_machine->mem_map();
    uint8_t *ptr = m_l1->map_addr(mem_map->brisc_firmware_base());
    memset(ptr, 0, mem_map->brisc_firmware_size());
    ptr = m_l1->map_addr(mem_map->ncrisc_firmware_base());
    memset(ptr, 0, mem_map->ncrisc_firmware_size());
    ptr = m_l1->map_addr(mem_map->trisc0_base());
    memset(ptr, 0, mem_map->trisc0_size());
#else
    for (int i = 0; i < 3; i++) {
