...
void LLK::pack_raw(uint32_t block_index, uint32_t output) {
    // special method: emulator only
    float *dst = get_dst_ptr(block_index);
    uint8_t *to = get_cb_write_ptr(output);
    uint32_t offset = m_cb->get_write_tile_ptr(output) * m_cb->get_tile_size(output);
    to += offset;
    m_cb->incr_write_tile_ptr(output, 1);
    pack_tile(get_cb_data_format(output), dst, to);
}
...
void LLK::unpack_AB(
    ...
    // TODO: Validate interpretation for COL and ROW broadcasts
    //     (there is apparent bug in respective llk_3c.h comments)
    switch (BType) {
