...
public:
    ...
    void copy_tile_to_dst_init_short_with_dt(
        ...
        uint32_t transpose) override;
#if 0 // TODO: Revise this
    void copy_tile_matmul_partials_init_short_with_dt(uint32_t cbid) override;
#endif
    void copy_tile(
        uint32_t in_cb_id, 
...
#if 0 // SKIPPED
    void graph_interpreter_init() override; 
    void get_next_op_info(op_info_t &op_info) override;
#endif
    ...
    void tanto_pack_reduce_rows_init(uint32_t ocb) override;
    void tanto_pack_reduce_cols_init(uint32_t ocb) override;
    void tanto_pack_reduce_scalar_init(uint32_t ocb) override;
private:
    Memory *m_l1;
    CB *m_cb;
    LLK m_llk;
