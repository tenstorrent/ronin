...
#include "tt_metal/impl/dispatch/device_command.hpp" // [RONIN] HostMemDeviceCommand
#include "tt_metal/impl/dispatch/command_queue.hpp" // [RONIN] NOC_X / NOC_y #defines only
...
void Program::compile(Device *device) {
...
#if 0 // [RONIN] TODO: Check whether this can be simply skipped
    ...
    for (auto &[core_type, kernels] : kernels_) {
        for (auto &[id, kernel] : kernels) {
            launch_build_step([kernel, device] { kernel->read_binaries(device); });
        }
    }
