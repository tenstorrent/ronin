...
#pragma once
//#include <boost/align/aligned_allocator.hpp> // [RONIN]
//#include <mutex> // [RONIN]

#include <immintrin.h> // [RONIN]
...
// [RONIN] (TODO: Check whether this works - may need replacement of intrinsics)
#if 0
template <typename T>
using vector_memcpy_aligned = std::vector<T, boost::alignment::aligned_allocator<T, MEMCPY_ALIGNMENT>>;
#else
template <typename T>
using vector_memcpy_aligned = std::vector<T>;
#endif
