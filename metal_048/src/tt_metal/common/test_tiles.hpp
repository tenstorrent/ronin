...
inline std::vector<T> convert_layout(
    ...
    switch (inL) {
        case TILED32_SWIZZLED:
            if (outL == TILED32_4FACES) {
                return convert_to_tile_layout<T, BufferType>(inp);
            } else if (outL == LIN_ROW_MAJOR) {
                return untilize_nchw<T, BufferType>(inp, shape);
            } else
                TT_ASSERT(false && "Unsupported conversion.");
        break;
        case LIN_ROW_MAJOR:
            if (outL == TILED32_SWIZZLED) {
                return tilize_nchw<T, BufferType>(inp, shape);
            } else if (outL == TILED32_4FACES) {
                auto swiz32 = convert_layout<T, BufferType>(inp, shape, inL, TILED32_SWIZZLED);
                return convert_layout<T, std::vector>(swiz32, shape, TILED32_SWIZZLED, outL);
            } else
                TT_ASSERT(false && "Unsupported conversion.");
        break;
        case TILED32_4FACES:
            if (outL == TILED32_SWIZZLED) {
                return convert_to_flat_layout<T, BufferType>(inp);
            } else if (outL == LIN_ROW_MAJOR) {
                auto swiz32 = convert_layout<T, BufferType>(inp, shape, inL, TILED32_SWIZZLED);
                return untilize_nchw<T, std::vector>(swiz32, shape);
            } else {
                TT_ASSERT(false && "Unsupported conversion");
            }
        break;
