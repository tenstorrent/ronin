...
//#include "hostdevcommon/dprint_common.h" // [RONIN]
#include "rtoptions.hpp"
//#include "third_party/umd/device/tt_silicon_driver_common.hpp" // [RONIN]
//#include "tools/profiler/profiler.hpp" // [RONIN]
//#include "tt_metal/impl/debug/sanitize_noc_host.hpp" // [RONIN]
#include "tt_metal/llrt/rtoptions.hpp"
#include "tt_metal/llrt/tlb_config.hpp"

#include "tt_metal/emulator/tt_emulator_device.h" // [RONIN]
...
void Cluster::detect_arch_and_target() {
    // [RONIN] SKIPPED: Support for Silicon and Versim
...
#endif
#endif
    this->target_type_ = TargetDevice::Model;
...
void Cluster::generate_cluster_descriptor() {
...
#endif
    // [RONIN] SKIPPED: Support for Silicon and Versim
    TT_FATAL(this->target_type_ == TargetDevice::Model);
    ...
    for (chip_id_t logical_mmio_device_id = 0; 
            logical_mmio_device_id < physical_mmio_device_ids.size();
            logical_mmio_device_id++) {
        logical_mmio_device_ids.insert(logical_mmio_device_id);
    }
    this->cluster_desc_ =
        tt_ClusterDescriptor::create_for_grayskull_cluster(
            logical_mmio_device_ids, physical_mmio_device_ids);

    std::set<chip_id_t> dummy_model_card = {0};
    this->devices_grouped_by_assoc_mmio_device_[0] = dummy_model_card;
    this->device_to_mmio_device_[0] = 0;
...
void Cluster::open_driver(
...
#endif
    TT_FATAL(this->target_type_ == TargetDevice::Model);
    device_driver = std::make_unique<tt_EmulatorDevice>(sdesc_path, this->cluster_desc_path_);
    ...
    this->get_metal_desc_from_tt_desc(
        device_driver->get_virtual_soc_descriptors(), 
        device_driver->get_harvesting_masks_for_soc_descriptors());
...
#else
    uint64_t noc_addr_local_bits_mask = (uint64_t(1) << NOC_ADDR_LOCAL_BITS) - 1;
#endif
