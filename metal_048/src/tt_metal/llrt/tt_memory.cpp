...
#if 0 // [RONIN]
// We use stoul to parse an address. We cast address_t to unsigned long long to format with %08lX.
static_assert(
...
#else
// We use stoull to parse an address. We cast address_t to unsigned long long to format with %08llX.
static_assert(
    numeric_limits<unsigned long long>::max() >= numeric_limits<memory::address_t>::max(),
    "unsigned long long can't cover whole range of addresses");

// We cast word_t to unsigned long long to format with %08llX.
static_assert(
    numeric_limits<unsigned long long>::max() >= numeric_limits<memory::word_t>::max(),
    "unsigned long long can't cover whole range of words");
#endif
