...
#include "tt_metal/third_party/umd/device/command_processor.h" // [RONIN]
...
class Cluster {
   public:
    ...
    // [RONIN]
    CommandProcessor *get_command_processor(chip_id_t device_id) const {
        return get_driver(device_id).get_command_processor();
    }
