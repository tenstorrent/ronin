...
#else
    gen_trisc_cpp(input_hlk_file_path, unpack_cpp, unpack_prolog);
    gen_trisc_cpp(input_hlk_file_path, math_cpp, math_prolog);
    gen_trisc_cpp(input_hlk_file_path, pack_cpp, pack_prolog);
#endif
...
#else
    generate_data_format_descriptors(options, env.get_arch());
    generate_math_fidelity_descriptor(options);
    generate_math_approx_mode_descriptor(options);
    generate_dst_accum_mode_descriptor(options);
#endif
}
