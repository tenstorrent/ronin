...
{
  // simple replacement for boost::algorithm::trim
  size_t n = s.size();
  size_t start = 0;
  while (start < n && std::isspace(s[start])) 
    {
      start++;
    }
  size_t end = n;
      ...
      while (iss)
        {
          ...
          if (addr < size_)
            {
//printf("@@@ loadHexFile-10 addr %zd val %x\n", addr, int(value)); 
              if (not errors)
                {
...
#if 0 // -A.G. (Apparent bug in original code)
      shift += 8;
#else
      shift += 7;
#endif
...
//printf("@@@  -> specInitByte addr %zd val %x\n", addr, int(value));
