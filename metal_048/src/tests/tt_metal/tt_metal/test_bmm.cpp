...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::vector<uint32_t> src0_vec = create_random_vector_of_bfloat16(bytesA, 1.0f, 0x1234);
        std::vector<uint32_t> src1_vec = create_random_vector_of_bfloat16(bytesB, 1.0f, 0x1234, -0.45f);
        tt_metal::EnqueueWriteBuffer(cq, src0_dram_buffer, src0_vec, false);
        tt_metal::EnqueueWriteBuffer(cq, src1_dram_buffer, src1_vec, false);
        ...
        std::vector<uint32_t> result_vec;
        tt_metal::EnqueueReadBuffer(cq, dst_dram_buffer, result_vec, true);
#endif
...
#else
            vector<uint16_t> src0_linear = convert_layout<uint16_t, vector>(u16_src0_vec, shapeA, TensorLayout::TILED32_4FACES, TensorLayout::LIN_ROW_MAJOR);
            vector<uint16_t> src1_linear = convert_layout<uint16_t, vector>(u16_src1_vec, shapeB, TensorLayout::TILED32_4FACES, TensorLayout::LIN_ROW_MAJOR);
#endif
...
#else
            auto gold_4f_u32 = u32_from_u16_vector( convert_layout<uint16_t, vector>(
                ref_bmm, shapeC, TensorLayout::LIN_ROW_MAJOR, TensorLayout::TILED32_4FACES));
#endif
