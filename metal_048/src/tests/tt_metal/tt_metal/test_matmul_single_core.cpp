...
#else
        auto activations_tile_layout = convert_to_tile_layout<bfloat16, std::vector>(activations_tilized);
#endif
...
#else
        tt_metal::EnqueueWriteBuffer(cq, src0_dram_buffer, activations_tile_transposed, false);
#endif
...
#else
        auto weights_tile_layout = convert_to_tile_layout<bfloat16, std::vector>(identity_tilized);
#endif
        auto weights = pack_bfloat16_vec_into_uint32_vec(weights_tile_layout);
#if 0 // [RONIN]
        tt_metal::detail::WriteToBuffer(src1_dram_buffer, weights);
#else
        tt_metal::EnqueueWriteBuffer(cq, src1_dram_buffer, weights, false);
#endif
...
#else
        auto result_flat_layout = convert_to_flat_layout<bfloat16, std::vector>(result_bfp16);
#endif
